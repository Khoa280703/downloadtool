...
export async function loadAdminDashboard(): Promise<AdminDashboardData> {
	...
	const [overviewResult, jobsResult, proxiesResult, activityResult] = await Promise.all([
	...
	return {
		overview: (overviewResult.rows[0] ?? {}) as AdminOverview,
		jobs: jobsResult.rows.map((row) => ({ ...row, updatedAt: normalizeDate(row.updatedAt) ?? row.updatedAt })),
		proxies: proxiesResult.rows.map((row) => ({
			...row,
