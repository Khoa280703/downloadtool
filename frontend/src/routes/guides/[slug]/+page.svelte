<script lang="ts">
	/**
	 * Guide detail page — renders a single ContentEntry with:
	 * quick answer box, content sections, FAQ, related guides, CTA.
	 */
...
<article class="max-w-3xl mx-auto px-6 pt-10 pb-16">
	...
	<nav aria-label="Breadcrumb" class="mb-6 flex gap-2 text-xs font-semibold text-plum/50">
		...
		<span>›</span>
		<a href="/guides" class="hover:text-primary transition-colors">{m.guides_breadcrumb()}</a>
		<span>›</span>
	...
	<div class="quick-answer rounded-2xl border border-primary/20 bg-pink-50 p-5 mb-10">
		<p class="text-xs font-bold text-primary uppercase tracking-wider mb-2">{m.content_quick_answer()}</p>
	...
	{#if related.length > 0}
		<section class="mt-10 pt-8 border-t border-pink-100">
			<h2 class="text-lg font-bold text-plum mb-4" style="font-family:'Fredoka',sans-serif">{m.content_related_guides()}</h2>
	...
	{#if data.entry.relatedMoneyPage}
		<section class="mt-10 rounded-2xl bg-gradient-to-r from-primary/10 to-secondary/10 border border-primary/20 p-6 text-center">
			<p class="text-sm font-bold text-plum mb-3">{m.content_cta_ready()}</p>
			<a
...
{#if data.entry.faqItems && data.entry.faqItems.length > 0}
	<KnowledgeSections
		...
		resourceLinks={related.map((r) => ({ href: `/guides/${r.slug}`, label: r.h1 }))}
		showHomeLink={true}
	/>
{/if}
